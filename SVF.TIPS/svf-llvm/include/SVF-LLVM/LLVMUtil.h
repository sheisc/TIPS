...
#if 1 // added by iron
bool isMemberFunctionCallSite(const CallBase* cs);
bool isCStyleIndirectCall(const CallBase* cs);
bool isLegalTargetFunc(const llvm::Function *func, const llvm::CallBase *callBase, const std::string &virFuncName);
bool isCompatibleType(const llvm::Type *ty1, const llvm::Type *ty2);
void printCppVirtualTable(const llvm::GlobalVariable *vtable);
const std::string getVirtualFuncName(const llvm::CallBase* callBase);
#endif
