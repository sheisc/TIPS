...
void SymbolTableBuilder::buildMemModel(SVFModule* svfModule)
{
    ...
    //PointerType *i8PtrTy = IntegerType::getInt8PtrTy(LLVMModuleSet::getLLVMModuleSet()->getContext());
    //Constant *i2pForZero = llvm::ConstantExpr::getIntToPtr(zeroVal, i8PtrTy);
    //collectSym(i2pForZero);
    collectSym(zeroVal);
#endif
...
#else
                    collectSym(st->getValueOperand());
                    const Value *storeVal = st->getValueOperand();
                    const Constant* C = SVFUtil::dyn_cast<Constant>(storeVal);
                    if (C && storeVal->getType()->isStructTy())
                    {
                        // store {i64, i64} val,  ptr
                        handleGlobalInitializerCE(C);
                    }
#endif
                }
...
#if 1 // added by iron
                else if (const ExtractValueInst* extractValInst =
                             SVFUtil::dyn_cast<ExtractValueInst>(&inst))\
                {
                    for (u32_t i = 0; i < extractValInst->getNumOperands(); i++)
                        collectSym(extractValInst->getOperand(i));

                    const Value *aggVal = extractValInst->getAggregateOperand();
                    const Constant* C = SVFUtil::dyn_cast<Constant>(aggVal);
                    if (C && aggVal->getType()->isStructTy())
                    {
                        /*
                         %memptr.adj = extractvalue { i64, i64 } { i64 ptrtoint @_ZN2BB3kkkEiii to i64, i64 0 }, 1
                         */
                        handleGlobalInitializerCE(C);
                    }
...
#if 1   // added by iron
        DBOUT(DMemModel, outs() << "\t\t##" << svfVal->toString() << " \n");
#endif
...
#if 1 // added by iron
            DBOUT(DMemModel, outs() << "\t\t##" << mem->getType()->getTypeInfo()->toString() << " \n");
            DBOUT(DMemModel, outs() << "\t\t##" << mem->getType()->toString() << " \n");
#endif
        }
    }
}
...
void SymbolTableBuilder::handleCE(const Value* val)
{
    if (const Constant* ref = SVFUtil::dyn_cast<Constant>(val))
    {
        ...
        else if (SVFUtil::isa<ConstantAggregate>(ref))
        {
            ...
            collectVal(ref);
#else
            DBOUT(DMemModelCE, outs() << "handle ConstantAggregate "
                  << LLVMModuleSet::getLLVMModuleSet()
                  ->getSVFValue(ref)
                  ->toString()
                  << "\n");
            collectVal(ref);
            // See void SymbolTableBuilder::buildMemModel(SVFModule* svfModule).
            // ReturnInst:
            //  ret { i64, i64 } { i64 ptrtoint (void (%class.DD*)* @_ZN2DD2ffEv to i64), i64 0 }
            llvm::StructType *st = SVFUtil::dyn_cast<llvm::StructType>(ref->getType());
            if (P2I2PCollector::isMemFuncFatPtrType(st, LLVMModuleSet::getLLVMModuleSet()->getMainLLVMModule()))
            {
                handleGlobalInitializerCE(ref);
            }
...
#if 0 // added by iron
            DBOUT(DGENERAL, val->print(llvm::outs()));
            DBOUT(DGENERAL, outs() << "\n");
#endif
            collectVal(ref);
        }
...
void SymbolTableBuilder::handleGlobalInitializerCE(const Constant* C)
{
...
#if 0 // added by iron
        DBOUT(DGENERAL, C->print(llvm::outs()));
        DBOUT(DGENERAL, outs() << "\n");
#endif
    }
#else
    ...
    {
        llvm::StructType *st = SVFUtil::dyn_cast<llvm::StructType>(C->getType());
        if (P2I2PCollector::isMemFuncFatPtrType(st, LLVMModuleSet::getLLVMModuleSet()->getMainLLVMModule()))
        {
            collectVal(C);
        }
