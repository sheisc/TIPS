...
hasConstantGEP (Value*  V)
{
    ...
    /*
        ...
        With the following code, we can:
        (1)  insert a ptrtoint instruction before the store
        (2)  fail to replace the ptrtoint CONSTANT exprestion with the ptrtoint instruction

     */
    if (ConstantAggregate * CA = SVFUtil::dyn_cast<ConstantAggregate>(V)) {
        for (unsigned index = 0; index < CA->getNumOperands(); ++index)
        {
            if (ConstantExpr * CE = hasConstantGEP(CA->getOperand(index))) {
                return CE;
            }
        }
        return 0;
    ...
    if (ConstantExpr * CE = SVFUtil::dyn_cast<ConstantExpr>(V))
    {
        ...
        else if (CE->getOpcode() == Instruction::PtrToInt || CE->getOpcode() == Instruction::IntToPtr)
        {
            return CE;
        }
        else
