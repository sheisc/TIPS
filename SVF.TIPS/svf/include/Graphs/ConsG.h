...
#if 0 // added by iron
    typedef Map<const NormalGepCGEdge *, const GepStmt *> NormalGepCGEdge2GepStmtMap;
#endif
...
#if 1 // added by iron
    //NormalGepCGEdge2GepStmtMap lazyComputedNormalCGGepToGepStmtMap;
    std::set<NodeID> lazyCGGepDstIds;
    std::set<const GepStmt *> lazyComputedGepStmts;
#endif
...
#if 1 // added by iron
    inline bool addLazyCGGepDstId(NodeID gepDstId)
    {
        return lazyCGGepDstIds.insert(gepDstId).second;
    }

    inline bool isLazyCGGepDstId(NodeID dstId)
    {
        return lazyCGGepDstIds.find(dstId) != lazyCGGepDstIds.end();
    }

    inline std::set<NodeID> & getLazyCGGepDstIds()
    {
        return lazyCGGepDstIds;
    }

    inline void updateLazyCGGepDstIds()
    {
        std::set<NodeID> repIds;
        for (NodeID nid: lazyCGGepDstIds)
        {
            repIds.insert(sccRepNode(nid));
        }
        //
        for (NodeID rid: repIds)
        {
            lazyCGGepDstIds.insert(rid);
        }
    }

    inline std::set<const GepStmt *> & getLazyComputedGepStmts()
    {
        return lazyComputedGepStmts;
    }

    inline bool addLazyComputedGepStmt(NodeID cgGepDstId, const GepStmt *pagGep)
    {
        lazyComputedGepStmts.insert(pagGep);
        addLazyCGGepDstId(cgGepDstId);
        return true;
    }
...
#else
    /// Get a field of a memory object
    inline NodeID getGepObjVar(NodeID id, const AccessPath& ls)
    {
        NodeID gep =  pag->getGepObjVar(id,ls);
        /// Create a node when it is (1) not exist on graph and (2) not merged
        if(sccRepNode(gep)==gep && hasConstraintNode(gep)==false)
