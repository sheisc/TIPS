...
protected:
    ...
    {
#if 1   // added by iron
        DBOUT(DGENERAL, SVFUtil::outs() << pagNode->toString() << "\n");
        DBOUT(DGENERAL, SVFUtil::outs() << node->toString() << "\n");
#endif
        ...
        {
#if 0 // added by iron,  see SVFIRBuilder::build()
            assert((it->second == node->getId()) && "a SVFVar can only have unique definition ");
#else
            // FIXME:
            // COPY edge override extractval instruction for modeling member function pointer
            PAGNodeToDefMap[pagNode] = node->getId();
#endif
        }
    }
    inline NodeID getDef(const PAGNode* pagNode) const
    {
        ...
        {
            // FIXME:
            return SymbolTableInfo::BlackHole;
        }
