...
#else
typedef s32_t APOffset;
typedef s32_t APByteOffset;
typedef s32_t APFieldIndex;
#endif
...
#if 1 // added by iron
    unsigned getNumOfElement() const
    {
        return numOfElement;
    }
#endif
...
#    define SVF_DEBUG_WITH_TYPE(TYPE, X)                                                    \
        do                                                                                  \
        {                                                                                   \
            if (SVFUtil::isEnabled(TYPE)) {                                                 \
                if (SVFUtil::isSVFDebugOutput())                                            \
                {                                                                           \
                    if (SVFUtil::outputFileAndLineNum())                                    \
                    {                                                                       \
