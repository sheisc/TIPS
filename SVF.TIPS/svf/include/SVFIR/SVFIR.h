...
{
    SVFValue *m_callbase;
    std::set<NodeID> targetFuncObjIds;

    SVFValue *m_funcPtr;
    SVFType *m_declaredFuncType;
};

struct VirtualCallContext: public IndirectCallContext
{
    SVFValue *m_objPtr;
    SVFValue *m_vtablePtr;
    int m_vtableIndex;
    SVFValue *m_funcPtr;

    SVFType *m_declaredObjType;
    SVFType *m_declaredFuncType;
    std::string m_virFuncName;


    VirtualCallContext(SVFValue *objPtr, SVFValue *vtablePtr, int vtableIndex,
                       SVFValue *virFuncPtr, SVFType *declaredObjType, SVFType *declaredFuncType,
                       SVFValue *callbase)
    {
        m_objPtr = objPtr;
        m_vtablePtr = vtablePtr;
        m_vtableIndex = vtableIndex;
        m_funcPtr = virFuncPtr;

        m_declaredObjType = declaredObjType;
        m_declaredFuncType = declaredFuncType;
...
struct MemberFuncCallContext: public IndirectCallContext
{
    // virtual member func call
    SVFValue *m_objPtr;
    SVFValue *m_vtablePtr;
    SVFValue *m_vtableIndex;
    SVFValue *m_virFuncPtr;

    // non-virtual member func call

    SVFValue *m_nonVirFuncPtr;
    ...
    MemberFuncCallContext(SVFValue *objPtr, SVFValue *vtablePtr, SVFValue *vtableIndex, SVFValue *virFuncPtr,
                          SVFValue *nonVirFuncPtr,
                          SVFValue *phiFuncPtr,
                          SVFType *declaredObjType,
                          SVFType *declaredFuncType,
                          SVFValue *callbase)
    {
        m_objPtr = objPtr;
        m_vtablePtr = vtablePtr;
        m_vtableIndex = vtableIndex;
        m_virFuncPtr = virFuncPtr;

        m_nonVirFuncPtr = nonVirFuncPtr;

        m_funcPtr = phiFuncPtr;
...
struct CStyleIndCallContext: public IndirectCallContext
{


    CStyleIndCallContext(SVFValue *funcPtr, SVFType *declaredFuncType, SVFValue *callbase)
    {
        m_funcPtr = funcPtr;
        m_declaredFuncType = declaredFuncType;
        m_callbase = callbase;
...
#if 1 // added by iron
    typedef std::map<NodeID, const SVFArrayType *> GepIndex2ArrayTypeMap;
    typedef std::map<NodeID, const SVFValue *> GepIndexNodeId2GepStmtSVFValueMap;
    typedef std::map<const GepStmt*, NodeID> GepStmt2GepIndexNodeIdMap;
#endif
...
#if 1 // added by iron
    GepIndex2ArrayTypeMap index2ArrayTypeMap;
    GepIndexNodeId2GepStmtSVFValueMap index2GepStmtMap;
    // inttoptr which have already been paired with ptrtoint
    std::set<const SVFValue*> allPtrToInts;
    std::set<const SVFValue*> allIntToPtrs;
    std::set<const SVFValue*> allIndirectCalls;
    std::set<const SVFValue*> allMemFuncPtrs;
    std::set<const SVFValue*> containerOfGeps;
    std::set<const GepStmt*> lazyComputedGeps;
    std::set<const GepStmt*> lazyComputedGepsForThisPointer;
    std::set<const GepStmt*> lazyComputedGepsForVtableIndex;
    GepStmt2GepIndexNodeIdMap gepStmt2IndexMapForThisPointer;
    GepStmt2GepIndexNodeIdMap gepStmt2IndexMapForVtableIndex;

    std::set<VirtualCallContext*> allVirtualCallContexts;
    std::set<MemberFuncCallContext*> allMemberFuncCallContexts;
    std::set<CStyleIndCallContext*> allCStyleIndCallContexts;
    std::map<NodeID, struct IndirectCallContext *> funcPtrId2IndCallContextMap;
#endif

public:

#if 1 // added by iron
    void printIndirectCallInfo();
    void printSVFValueByID(NodeID nid);

    void collectMemberFuncCallInfo();
    void collectVirtualCallInfo();
    void collectCStyleIndirectCallInfo();

    void refineTargetFuncObjIds(SVFValue *svfCallBase, std::set<NodeID> &targetFuncObjIds, const std::string &virFuncName);

    bool getVirtualFunctionByVtableIndex(const SVFValue *vtable, long vtableIdx, std::set<NodeID> &virFuncObjIds);

    std::set<long> & getAllVirMemFuncVtableIndexes();

    std::set<NodeID> & getAllNonVirMemFuncObjIds();

    inline std::map<NodeID, struct IndirectCallContext *> & getFuncPtrId2IndCallContextMap()
    {
        return funcPtrId2IndCallContextMap;
    }

    inline std::set<VirtualCallContext*> & getAllVirtualCallContexts()
    {
        return allVirtualCallContexts;
    }

    inline std::set<MemberFuncCallContext*> & getAllMemberFuncCallContexts()
    {
        return allMemberFuncCallContexts;
    }

    inline std::set<CStyleIndCallContext*> & getAllCStyleIndCallContexts()
    {
        return allCStyleIndCallContexts;
    }

    inline bool addVirtualCallContext(VirtualCallContext *context)
    {
        return allVirtualCallContexts.insert(context).second;
    }

    inline bool addMemberFuncCallContext(MemberFuncCallContext *context)
    {
        return allMemberFuncCallContexts.insert(context).second;
    }

    inline bool addCStyleIndCallContext(CStyleIndCallContext *context)
    {
        return allCStyleIndCallContexts.insert(context).second;
    }

    inline GepStmt2GepIndexNodeIdMap & getGepStmt2IndexMapForThisPointer()
    {
        return gepStmt2IndexMapForThisPointer;
    }

    inline GepStmt2GepIndexNodeIdMap & getGepStmt2IndexMapForVtableIndex()
    {
        return gepStmt2IndexMapForVtableIndex;
    }

    inline GepIndex2ArrayTypeMap & getIndex2ArrayTypeMap()
    {
        return index2ArrayTypeMap;
    }

    inline GepIndexNodeId2GepStmtSVFValueMap & getIndex2GepStmtMap()
    {
        return index2GepStmtMap;
    }

    inline std::set<const SVFValue*> & getAllPtrToInts()
    {
        return allPtrToInts;
    }

    inline std::set<const SVFValue*> & getAllIntToPtrs()
    {
        return allIntToPtrs;
    }

    inline std::set<const SVFValue*> & getAllIndirectCalls()
    {
        return allIndirectCalls;
    }

    inline std::set<const SVFValue*> & getAllMemFuncPtrs()
    {
        return allMemFuncPtrs;
    }

    inline std::set<const SVFValue*> & getContainerOfGeps()
    {
        return containerOfGeps;
    }

    inline std::set<const GepStmt*> & getLazyComputedGeps()
    {
        return lazyComputedGeps;
    }

    inline std::set<const GepStmt*> & getLazyComputedGepsForThisPointer()
    {
        return lazyComputedGepsForThisPointer;
    }

    inline std::set<const GepStmt*> & getLazyComputedGepsForVtableIndex()
    {
        return lazyComputedGepsForVtableIndex;
    }

    inline bool isLazyComputedGep(const GepStmt *gep)
    {
        return lazyComputedGeps.find(gep) != lazyComputedGeps.end();
    }

    inline bool isLazyComputedGepForThisPointer(const GepStmt *gep)
    {
        return lazyComputedGepsForThisPointer.find(gep) != lazyComputedGepsForThisPointer.end();
    }

    inline bool isLazyComputedGepForVtableIndex(const GepStmt *gep)
    {
#if 1
        return lazyComputedGepsForVtableIndex.find(gep) != lazyComputedGepsForVtableIndex.end();
#else
        NodeID src1 = gep->getRHSVarID();
        NodeID dst1 = gep->getLHSVarID();
        for (const GepStmt *g : lazyComputedGepsForVtableIndex)
        {
            NodeID src2 = g->getRHSVarID();
            NodeID dst2 = g->getLHSVarID();
            if (src2 == src1 && dst1 == dst2){
                return true;
            }
        }
        return false;
...
//    inline bool getLazyComputedIndexNodeId(const GepStmt *gep, NodeID &indexNodeId)
//    {
//        if (isLazyComputedGepForThisPointer(gep))
//        {
//            indexNodeId = gepStmt2IndexMapForThisPointer[gep];
//            return true;
//        }
//        else if (isLazyComputedGepForVtableIndex(gep))
//        {
//            indexNodeId = gepStmt2IndexMapForVtableIndex[gep];
//            return true;
//        }
//        return false;
//    }
    inline bool addContainerOfGep(const SVFValue *gep)
    {
        return containerOfGeps.insert(gep).second;
    }

    inline bool addLazyComputedGep(const GepStmt *gep)
    {
        return lazyComputedGeps.insert(gep).second;
    }

    inline bool addLazyComputedGepForThisPointer(const GepStmt *gep)
    {
        return lazyComputedGepsForThisPointer.insert(gep).second;
    }

    inline bool addLazyComputedGepForVtableIndex(const GepStmt *gep)
    {
        return lazyComputedGepsForVtableIndex.insert(gep).second;
    }

    inline bool addGepStmtAndIndexForThisPointer(const GepStmt *gep, NodeID indexNodeId)
    {
        gepStmt2IndexMapForThisPointer[gep] = indexNodeId;
        addLazyComputedGep(gep);
        addLazyComputedGepForThisPointer(gep);
        return true;
    }

    inline bool addGepStmtAndIndexForVtableIndex(const GepStmt *gep, NodeID indexNodeId)
    {
        gepStmt2IndexMapForVtableIndex[gep] = indexNodeId;
        addLazyComputedGep(gep);
        addLazyComputedGepForVtableIndex(gep);
        return true;
    }

    inline bool addPtrToInt(const SVFValue *p2i)
    {
        return allPtrToInts.insert(p2i).second;
    }

    inline bool addIntToPtr(const SVFValue *i2p)
    {
        return allIntToPtrs.insert(i2p).second;
    }

    inline bool addMemFuncPtr(const SVFValue *memFuncPtr)
    {
        return allMemFuncPtrs.insert(memFuncPtr).second;
    }

    inline bool addOneIndirectCall(const SVFValue *indirectCall)
    {
        return allIndirectCalls.insert(indirectCall).second;
    }

    inline bool isPtrToInt(const SVFValue *p2i)
    {
        return allPtrToInts.find(p2i) != allPtrToInts.end();
    }

    inline bool isIntToPtr(const SVFValue *i2p)
    {
        return allIntToPtrs.find(i2p) != allIntToPtrs.end();
    }

    inline bool isContainerOfGep(const SVFValue *gep)
    {
        return containerOfGeps.find(gep) != containerOfGeps.end();
    }

    inline bool addIndexAndGepStmt(NodeID nid, const SVFValue* gepStmt)
    {
        index2GepStmtMap[nid] = gepStmt;
        return true;
    }

    inline bool addIndexAndArrayPair(NodeID nid, const SVFArrayType *arrayTy)
    {
        if (index2ArrayTypeMap.find(nid) == index2ArrayTypeMap.end())
        {
            index2ArrayTypeMap[nid] = arrayTy;
            return true;
        }
        else
        {
    ...
    bool isIntAsPtrSVFFunc(const SVFFunction *svfFunc);
#endif
...
#else
    /// Get a field SVFIR Object node according to base mem obj and offset
    NodeID getGepObjVar(const MemObj* obj, const AccessPath& ls);
    /// Get a field obj SVFIR node according to a mem obj and a given offset
    NodeID getGepObjVar(NodeID id, const AccessPath& ls) ;
    APFieldIndex getFieldIndexByByteOffset(const SVFType *svfTy, APByteOffset btOffset);
#endif
...
#else
    /// Add a field obj node, this method can only invoked by getGepObjVar
    NodeID addGepObjNode(const MemObj* obj, const AccessPath& ls);
#endif
