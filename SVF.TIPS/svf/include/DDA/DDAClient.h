...
public:
    ...
    virtual inline OrderedNodeSet& collectCandidateQueries(SVFIR* p)
    {
        ...
        {
            pipMode = true;
            for (const SVFValue *indirecCall: p->getAllIndirectCalls())
            {
                NodeID nodeId = p->getValueNode(indirecCall);
                addCandidate(nodeId);
            }
        }
        //
        ...
        {
            pipMode = true;
            SVFIR::GepIndex2ArrayTypeMap &  index2ArrayTyMap = p->getIndex2ArrayTypeMap();
            for (auto i2a: index2ArrayTyMap)
            {
                addCandidate(i2a.first);
            }
        }
        if (!pipMode)
        {
