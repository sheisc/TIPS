...
{
#if 1   // added by iron
    DBOUT(DGENERAL, outs() << getPAG()->getGNode(edge->getDstID())->toString() << "\n");
#endif
...
#if 0
    // Debug for CPU2006 453.povray. Fxxx the C++ Member Function Ptr
    if (F->getName().find("_ZN21POVMS_MessageReceiver14ReceiveHandlerEP9POVMSDataS1_iPv") != std::string::npos )
    {
        //DBOUT(DAndersen, outs() << F->getName() << "\n");
        outs() << __FILE__ << " " << __LINE__ << F->getName() << "\n";
    }
    // _ZN21POVMS_MessageReceiver15MemberHandlerOOIN12pov_frontend14RenderFrontendEE4CallER13POVMS_MessageS5_i
    if (F->getName().find("_ZN21POVMS_MessageReceiver15MemberHandlerOOIN12pov_frontend14RenderFrontendEE4CallER13POVMS_MessageS5_i") != std::string::npos )
    {
        //DBOUT(DAndersen, outs() << F->getName() << "\n");
        outs() << __FILE__ << " " << __LINE__ << F->getName() << "\n";
    }
