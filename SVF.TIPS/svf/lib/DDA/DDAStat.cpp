...
void DDAStat::performStatPerQuery(NodeID ptr)
{
    ...
    if(flowDDA)
    {
        ...
        if (Options::PipMemFuncPtrs()){
            ...
            PointsTo tmpDstPts;  // to be cleared
            for (auto piter: flowDDA->getPts(ptr))
            {
                NodeID nid = piter;
                const PAGNode* node = getPTA()->getPAG()->getGNode(nid);
                if (!node->hasValue())
                {
                    tmpDstPts.set(nid);
                    continue;
                ...
                const SVFFunction *func = SVFUtil::dyn_cast<SVFFunction>(pointeeVal);
                if (!func || (func->getName() == "__cxa_pure_virtual"))
                {
                    tmpDstPts.set(nid);
                }
            }
            for(auto nid: tmpDstPts)
            {
                flowDDA->clearPts(ptr, nid);
            }
        }
#endif
        cptsSize = flowDDA->getPts(ptr).count();
    ...
    else if(contextDDA)
    {
        ...
        if (Options::PipMemFuncPtrs()){
            ...
            CxtPtSet tmpDstPts; // to be cleared
            for (auto piter : contextDDA->getPts(var))
            {
                CxtVar ptd = piter;
            ...
            for(auto piter: tmpDstPts)
            {
                contextDDA->getPTDataTy()->clearPts(var, piter);
            }
        }
#endif
        cptsSize = contextDDA->getPts(var).count();
...
#else
    PTAStat::printStat("Demand-Driven Pointer Analysis Statistics");
#endif
}
