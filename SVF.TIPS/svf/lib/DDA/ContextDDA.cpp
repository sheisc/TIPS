 ...
 * Generate field objects for structs
 */
CxtPtSet ContextDDA::processGepPts(const GepSVFGNode* gep, const CxtPtSet& srcPts, const CxtLocDPItem& dpm)
{
    CxtPtSet tmpDstPts;
        ...
        else
        {
            ...
            else
            {
                if (pag->isLazyComputedGepForThisPointer(gepStmt))
                {
                    ...
                    CxtVar indexVar(dpm.getCondVar().get_cond(), indexNodeId);
                    PAGNode* indexNode = getPAG()->getGNode(indexNodeId);
                    CxtLocDPItem indexDpm = getDPIm(indexVar, getDefSVFGNode(indexNode));
                    const PointsTo &indexPts = getBVPointsTo(findPT(indexDpm));
                    //const CxtPtSet &indexPts = getPts(indexVar);
                ...
                else if (pag->isLazyComputedGepForVtableIndex(gepStmt))
                {
                    ...
                    {
//                        NodeID vtableId = ptd.get_id();
//                        const PAGNode* vtableNode = pag->getGNode(vtableId);
...
//                        }
                ...
                {
                    CxtVar var(ptd.get_cond(),getGepObjVar(ptd.get_id(), gepStmt->getAccessPath()));
                    tmpDstPts.set(var);
                }
            }
        }
