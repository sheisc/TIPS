...
#else
NodeID SVFIR::getGepObjVar(NodeID id, const AccessPath& ls)
{
    SVFVar* node = pag->getGNode(id);
    if (GepObjVar* gepNode = SVFUtil::dyn_cast<GepObjVar>(node))
    {
        AccessPath accPath = gepNode->getAccessPath() + ls;
        const MemObj *memObj = gepNode->getMemObj();
        if (ls.readjustNeeded())
        {
            APByteOffset btOffset = accPath.getAccessPathByteOffset();
            const SVFType *svfTy = memObj->getType();
            APFieldIndex fldIndex = getFieldIndexByByteOffset(svfTy, btOffset);
            accPath.setAccessPathFieldIndex(fldIndex);
        }
        return getGepObjVar(memObj, accPath);
    }
    else if (FIObjVar* baseNode = SVFUtil::dyn_cast<FIObjVar>(node))
    {
        if (ls.readjustNeeded())
        {
            AccessPath accPath(ls);
            const MemObj *memObj = baseNode->getMemObj();
            APByteOffset btOffset = ls.getAccessPathByteOffset();
            const SVFType *svfTy = memObj->getType();
            APFieldIndex fldIndex = getFieldIndexByByteOffset(svfTy, btOffset);
            accPath.setAccessPathFieldIndex(fldIndex);
            return getGepObjVar(baseNode->getMemObj(), accPath);
        }
        else
        {
            return getGepObjVar(baseNode->getMemObj(), ls);
...
#if 1 // added by iron
    DBOUT(DMemModel, outs() << "\t\t##" << obj->getType()->getTypeInfo()->toString() << " \n");
    DBOUT(DMemModel, outs() << "\t\t##" << obj->getType()->toString() << " \n");
#endif
...
#else
NodeID SVFIR::getGepObjVar(const MemObj* obj, const AccessPath& ls)
{
    NodeID base = obj->getId();
#if 1 // added by iron
    DBOUT(DMemModel, outs() << "\t\t##" << obj->getType()->getTypeInfo()->toString() << " \n");
    ...
    AccessPath newLS = pag->getSymbolInfo()->getModulusOffset(obj,ls);

    // Base and first field are the same memory location.
    if (Options::FirstFieldEqBase() && newLS.getConstantFieldIdx() == 0) return base;

    NodeOffsetMap::iterator iter = GepObjVarMap.find(std::make_pair(base, newLS.getConstantFieldIdx()));
    if (iter == GepObjVarMap.end())
        return addGepObjNode(obj, newLS);
    else
        return iter->second;
...
#else
NodeID SVFIR::addGepObjNode(const MemObj* obj, const AccessPath& ls)
{
    //assert(findPAGNode(i) == false && "this node should not be created before");
    NodeID base = obj->getId();
    assert(0==GepObjVarMap.count(std::make_pair(base, ls.getConstantFieldIdx()))
           && "this node should not be created before");

    NodeID gepId = NodeIDAllocator::get()->allocateGepObjectId(base, ls.getConstantFieldIdx(), Options::MaxFieldLimit());
    GepObjVarMap[std::make_pair(base, ls.getConstantFieldIdx())] = gepId;
    GepObjVar *node = new GepObjVar(obj, gepId, ls);
    memToFieldsMap[base].set(gepId);
    return addObjNode(obj->getValue(), node, gepId);
}
...
bool SVFIR::isValidTopLevelPtr(const SVFVar* node)
{
    ...
    if (SVFUtil::isa<ValVar>(node))
    {
        if (isValidPointer(node->getId()) && node->hasValue())
        {
            ...
            {
                return pag->isIntToPtr(svfVal);
            }
            return !SVFUtil::isArgOfUncalledFunction(svfVal);
        }
    }
    return false;
#endif
