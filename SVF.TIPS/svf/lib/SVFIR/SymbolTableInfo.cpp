 ...
 * Get modulus offset given the type information
 */
AccessPath SymbolTableInfo::getModulusOffset(const MemObj* obj, const AccessPath& ls)
{
    /// if the offset is negative, it's possible that we're looking for an obj node out of range
    /// of current struct. Make the offset positive so we can still get a node within current
    /// struct to represent this obj.

    APOffset offset = ls.getConstantFieldIdx();
    u32_t maxOffset = obj->getMaxFieldOffsetLimit();
    if(offset < 0)
    {
        writeWrnMsg("try to create a gep node with negative offset.");
    ...
    //AccessPath accPath(offset, ls.getAccessPathByteOffset());
    AccessPath accPath(ls);
    accPath.setFieldIndexAndByteOffset(offset, ls.getAccessPathByteOffset());
    return accPath;
}
#endif
...
#if 1 // added by iron
void SymbolTableInfo::printSVFTypes()
{
    for (const SVFType *st: svfTypes)
    {
        outs() << "..........................." << "\n";
        this->printFlattenFields(st);
        const StInfo* info = this->getTypeInfo(st);
        outs() << info->toString(2) << "\n";
    }
}
#endif
...
//    {
//        if (SVFUtil::isMemFuncPtrStruct(typeInfo->getType()))
//        {
//            return true;
//        }
//        else
