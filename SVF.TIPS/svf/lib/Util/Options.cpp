...
#if 1   // added by iron
const Option<bool> Options::PipPreOnly(
    "pip-pre-only",
    "Only preprocess IRs for ptrtoint and inttoptr",
    false
);

const Option<bool> Options::CompositionAnalysisOnly(
    "composition-analysis-only",
    "Only analyze composition relationship",
    false
);

const Option<bool> Options::CheckingVirFuncName(
    "detect-virtual-func-name",
    "Check virtual function names",
    false
);

const Option<bool> Options::PipArrayOOB(
    "pip-array-oob",
    "Detect array ouf of bound.",
    false
);

const Option<bool> Options::PIPDebugOutput(
    "pip-debug-output",
    "Debug output",
    false
);

const Option<bool> Options::PIPOutputFileAndLineNum(
    "pip-output-file-line",
    "Ouput file name and line number",
    false
);

const Option<bool> Options::OnlyI2PPtsPrint(
    "only-print-i2p-pts",
    "Only Print points-to set for inttoptr pointers",
    false
);

const Option<bool> Options::PipAddI2PQuery(
    "pip-add-i2p-query",
    "Add inttoptr queries for DDA",
    false
);

const Option<bool> Options::PipIndirectCalls(
    "pip-indirect-calls",
    "Add DDA queries for indirect calls",
    false
);

const Option<bool> Options::PipMemFuncPtrs(
    "pip-memfuncptrs",
    "Add DDA queries for C++ member function pointers",
    false
);

const Option<bool> Options::PipPrintStat(
    "pip-print-stat",
    "Print stats in PIP",
    false
);

const Option<bool> Options::IntegerAsPointer(
    "int-as-pointer",
    "Treat integer types as pointer types",
    true
);

const Option<bool> Options::SelectiveIntegerAsPointer(
    "selective-int-as-pointer",
    "Treat integer types as pointer types seletively",
    false
);

const Option<bool> Options::VirtualCallAsIndirectCall(
    "vcall-as-indirect-call",
    "Treat C++ virtual calls as normal indirect calls",
    false
);

const Option<bool> Options::AllAsPointer(
    "all-as-pointer",
    "Treat all primary types as pointer types",
    false
);

const Option<bool> Options::EnableLazyDiamond(
    "enable-lazy-diamond",
    "Enale lazy computing for virtual inheritance",
    false
);
#endif
