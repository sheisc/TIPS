...
#if 1 // added by iron
bool SVFUtil::isSVFDebugOutput()
{
    return Options::PIPDebugOutput();
}

bool SVFUtil::outputFileAndLineNum()
{
    return Options::PIPOutputFileAndLineNum();
}

bool SVFUtil::enableAllAsPointer()
{
    return Options::AllAsPointer();
}

bool SVFUtil::enableIntegerAsPointer()
{
    return Options::IntegerAsPointer();
}

bool SVFUtil::isOnlyI2PPtsPrint()
{
    return Options::OnlyI2PPtsPrint();
}

unsigned SVFUtil::getSizeOfPointerType()
{
    return sizeof(char *);
}

bool SVFUtil::isEnabled(const char *t)
{
    static const char * debugTargets[] =
    {
#if 0
        DGENERAL,
        DPAGBuild,
    ...
    //
    for (unsigned long i = 0; i < sizeof(debugTargets)/sizeof(debugTargets[0]); ++i)
    {
        if (strcmp(debugTargets[i], t) == 0)
        {
            return true;
        }
    }
