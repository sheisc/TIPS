...
%struct.EE = type { %class.BB.base, %class.CC.base, [2 x i64], %class.AA }
...
One class have only one _ZTV vtable and 0+ _ZTC vtable
(1)
iron@OPTI:~$ c++filt _ZTC2EE16_2CC
construction vtable for CC-in-EE
(2)
iron@OPTI:~$ c++filt _ZTC2DD16_2CC
construction vtable for CC-in-DD
(3)
iron@OPTI:~$ c++filt _ZTV2CC
vtable for CC

 */
const std::string constructionVtableAfterDemangle = "construction vtable for ";
#endif
...
std::string cppUtil::getClassNameFromVtblObj(const std::string& vtblName)
{
    ...
    if (realname != nullptr)
    {
        ...
        else if (realnameStr.compare(0, constructionVtableAfterDemangle.size(),
                                     constructionVtableAfterDemangle) == 0)
        {
            // FIXME: _ZTC
            // construction vtable for CC-in-DD
            className = realnameStr.substr(constructionVtableAfterDemangle.size());
            std::size_t pos = className.find("-in-");
            if (pos != std::string::npos)
            {
                std::size_t len = className.length();
                std::size_t ddPos = pos + strlen("-in-");
                std::size_t ddLen = len - ddPos;
                std::string ccName = className.substr(0, pos);
                std::string ddName = className.substr(ddPos, ddLen);
                (void) ddName;
                //std::cout << ccName << " : " << ddName << std::endl;
                className = ccName;
            }
        }
        std::free(realname);
