...
void ConstraintGraph::buildCG()
{
                ...
                ngeps.end(); iter != eiter; ++iter)
    {
        ...
        {
            NormalGepCGEdge *cgGep = addNormalGepCGEdge(edge->getRHSVarID(),edge->getLHSVarID(),edge->getAccessPath());
            if (cgGep && pag->isLazyComputedGep(edge))
            {
                addLazyComputedGepStmt(cgGep->getDstID(), edge);
            }
        }
#endif
    }
    ...
    static std::string getNodeLabel(NodeType *n, ConstraintGraph*)
    {
        ...
        else
        {
            ...
            if (!SVFUtil::isa<DummyValVar>(node) && !SVFUtil::isa<DummyObjVar>(node))
                ...
                rawstr << node->getId() << ":";
#if 1   // added by iron
            if (GepObjVar *gepObj = SVFUtil::dyn_cast<GepObjVar>(node))
            {
               rawstr << "GepObjVar:  with offset_" << ((s32_t)gepObj->getConstantFieldIdx()) << "\n";
            }
#endif
