...
#else
    getPAG()->getSymbolInfo()->printSVFTypes();
#endif
}
...
void PointerAnalysis::dumpPts(NodeID ptr, const PointsTo& pts)
{
    ...
    for (PointsTo::iterator it = pts.begin(), eit = pts.end(); it != eit; ++it)
    {
        ...
        if (node->hasValue())
        {
            if (const SVFFunction *func = SVFUtil::dyn_cast<SVFFunction>(node->getValue()))
            {
                if (SVFUtil::isVirtualFunction(func))
                {
                    numVirt++;
                    //outs() << "#TIPSVirtualFunc# ";
                }
                else if (SVFUtil::isThunkFunction(func))
                {
                    numThunk++;
                    //outs() << "#TIPSThunkFunc# ";
                }
                else if (SVFUtil::isNonVirtualFunction(func))
                {
                    numNonVirt++;
                    //outs() << "#TIPSNonVirtualFunc# ";
                }
                else
                {
                    otherFunc++;
...
//    outs() << "\n#TIPSNumberOfFuncs# Virtal:Thunk:Non-Virtual:Other:  "
//           << numVirt << " " << numThunk << " " << numNonVirt << " " << otherFunc << "\n\n";
#endif
}
