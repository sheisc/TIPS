...
#if 0   // added by iron
void BVDataPTAImpl::dumpI2PPtsTo()
{
...
#if 0
    OrderedNodeSet pagNodes;
    std::set<const SVFFunction *> svfFuncs;    
    for(SVFIR::iterator it = pag->begin(), eit = pag->end(); it!=eit; it++)
    {
        SVFVar *pagNode = pag->getGNode(it->first);
        if (pagNode->hasValue())
        {
            if (const SVFInstruction *inst = SVFUtil::dyn_cast<SVFInstruction>(pagNode->getValue()))
            {
                // POVMS_MessageReceiver::ReceiveHandler(POVMSData*, POVMSData*, int, void*), // <453, povmscpp.cpp, 994>
                const std::string &name = inst->getName();
                if (name.find("TIPS_") != std::string::npos
                        || inst->getFunction()->getName().find("_ZN21POVMS_MessageReceiver14ReceiveHandlerEP9POVMSDataS1_iPv")
                                != std::string::npos)
                {
                    const SVFFunction *svfFunc = inst->getFunction();
                    if (svfFuncs.insert(svfFunc).second)
                    {
                        for (unsigned i = 0; i < svfFunc->arg_size(); ++i)
                        {
                            NodeID nid = pag->getValueNode(svfFunc->getArg(i));
                            pagNodes.insert(nid);
                        }
                    }
                }
    ...
    for(SVFIR::iterator it = pag->begin(), eit = pag->end(); it!=eit; it++)
    {
        ...
        if (Options::OnlyI2PPtsPrint())
        {
            if (pagNode->hasValue())
            {
                ...
                if (pag->isIntToPtr(svfVal))
                {
                    if (getPts(it->first).count() > 0)
                    {
                        outs() << "\n" << "#TIPSIntToPtr#" << svfVal->toString() << "\n";
                    }
                    else
                    {
                        outs() << "\n" << "#TIPSIntToPtrWarning#" << svfVal->toString() << "\n";
                    }
                    pagNodes.insert(it->first);
                }
                else if (const SVFInstruction *inst = SVFUtil::dyn_cast<SVFInstruction>(svfVal))
                {
                    const SVFFunction *f = inst->getFunction();
                    if (svfFuncs.find(f) != svfFuncs.end())
                    {
                        pagNodes.insert(it->first);
                    }
                }
...
void BVDataPTAImpl::onTheFlyCallGraphSolve(const CallSiteToFunPtrMap& callsites, CallEdgeMap& newEdges)
{
    ...
    {
        const CallICFGNode* cs = iter->first;
        NodeID indFuncPtrId = iter->second;


//        NodeID callNodeId = pag->getValueNode(cs->getCallSite());
//        if (callNodeId == 161997)
//        {
//            outs() << __FILE__ << " " << __LINE__ << " " << cs->getCallSite()->toString() << "\n";
//            for (NodeID f: getPts(iter->second))
//            {
//                PAGNode * n = pag->getGNode(f);
//                if (n->hasValue())
//                {
//                    outs() << __FILE__ << " " << __LINE__ << " " << n->getValue()->toString() << "\n";
//                }
//            }
//        }
        //pag->printSVFValueByID(indFuncPtrId);
#if 1
        std::map<NodeID, struct IndirectCallContext *> &funcId2ContextMap = pag->getFuncPtrId2IndCallContextMap();
        if (funcId2ContextMap.count(indFuncPtrId) > 0)
        {
            struct IndirectCallContext *icContext = funcId2ContextMap[indFuncPtrId];
            PointsTo illegalTargets;
            for (NodeID curFuncId: getPts(indFuncPtrId))
            {
                if (icContext->targetFuncObjIds.count(curFuncId) <= 0)
                {
                    illegalTargets.set(curFuncId);
                }
                else
                {
                    //pag->printSVFValueByID(curFuncId);

                }
            }
            for (NodeID illegalFuncId: illegalTargets)
            {
                //pag->printSVFValueByID(illegalFuncId);
                clearPts(indFuncPtrId, illegalFuncId);
            }
            illegalTargets.clear();
        }
#endif
        resolveIndCalls(cs, getPts(indFuncPtrId), newEdges);
